...
# get the role out of the security table with the key
...
def require_api_role(required_role):
    ...
    def decorator(f):
        ...
        def decorated(*args, **kwargs):
            #code to check key
            key = request.headers.get('X-API-KEY') or request.args.get('api_key') #remove request args in real as it leaks the api key just for test
            apikey = get_api_key()
